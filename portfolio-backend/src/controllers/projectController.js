...
export async function createProject(req, res) {
  try {
    const { title, description, tech, githubUrl, demoUrl, featured } = req.body;
    ...
    if (req.file) {
      payload.coverImage = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
    }
...
export async function updateProject(req, res) {
  try {
    ...
    if (req.file) {
      updates.coverImage = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
    }
